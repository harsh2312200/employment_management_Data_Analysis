...
select Top 7 FirstName, LastName, HireDate from Employee
order by  Hiredate DESC


-- 4. Question: Get the count of employees in each job title.

Select JobTitleName, COunt(EmployeeID) as Noofemployess from Employee E
inner join JobTitle J
...
select CONCAT (FirstName, ' ',LastName), DateOfBirth, Gender, PhoneNumber, Email  from Employee E
...
Select CONCAT(FirstName,' ',LastName), DepartmentName from Employee E
Inner join Department D
...
Select FirstName, ProjectName as Project_Name, JobTitleName as Project_Role from Employee E
...
Select DepartmentName, COunt(EmployeeID) as Noofemployess from Employee E
inner join Department J
on j.DepartmentID = E.JobTitleID
...
Select  CONCAT(E.FirstName, ' ', E.LastName) as 'Employee Name',
        CONCAT(Em.FirstName, ' ', Em.LastName) as 'Manager Name'
FROM Employee E 
...
Select  CONCAT(Em.FirstName, ' ', Em.LastName) as 'Manager Name',
...
Select CONCAT(FirstName,' ',LastName) As Name, JobTitleName, StartDate from Employee E
inner join ProjectAllocation PA
on E.EmployeeID = PA.EmployeeID
Inner join Project P
on P.ProjectID = PA.ProjectID
...
Select CONCAT(FirstName,' ',LastName) As Name, ProjectName, StartDate from Employee E
inner join ProjectAllocation PA
on E.EmployeeID = PA.EmployeeID
Inner join Project P
on P.ProjectID = PA.ProjectID
...
Select CONCAT(FirstName,' ',LastName) as FullName,DepartmentName, (BaseSalary + Bonus) - Deductions AS Total_Salary
from Employee E
Inner Join Department D
On D.DepartmentID = E.DepartmentID
Inner Join Salary S
...
select Concat(FirstName,' ',LastName) As FullName,DateofBirth, (Year(GetDate()) -  Year(DateOfBirth)) as Age from Employee
